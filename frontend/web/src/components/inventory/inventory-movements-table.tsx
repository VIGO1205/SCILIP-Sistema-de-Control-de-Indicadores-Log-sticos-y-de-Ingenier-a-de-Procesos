...
import { History, ArrowDownLeft, ArrowUpRight, RefreshCw, ArrowRightLeft, ChevronLeft, ChevronRight } from 'lucide-react';
...
const TYPE_MAP: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  IN: { label: 'Entrada', color: 'emerald', icon: <ArrowDownLeft className="h-3.5 w-3.5" /> },
  OUT: { label: 'Salida', color: 'red', icon: <ArrowUpRight className="h-3.5 w-3.5" /> },
  ADJUSTMENT: { label: 'Ajuste', color: 'amber', icon: <RefreshCw className="h-3.5 w-3.5" /> },
  TRANSFER: { label: 'Traslado', color: 'blue', icon: <ArrowRightLeft className="h-3.5 w-3.5" /> },
};

function getTypeInfo(type: string | undefined) {
  if (!type) return { label: 'Desconocido', color: 'gray', icon: <History className="h-3.5 w-3.5" /> };
  return TYPE_MAP[type.toUpperCase()] || { label: type, color: 'gray', icon: <History className="h-3.5 w-3.5" /> };
}
