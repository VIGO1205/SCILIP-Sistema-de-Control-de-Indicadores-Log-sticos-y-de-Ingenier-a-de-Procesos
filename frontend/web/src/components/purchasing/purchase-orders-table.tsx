...
const STATUS_MAP: Record<string, { label: string; colorClass: string }> = {
  ...
  CANCELLED: { label: 'Cancelado',  colorClass: 'bg-gray-50 text-gray-500 dark:text-dark-tremor-content-subtle border-gray-200 dark:border-dark-tremor-border' },
  DRAFT:     { label: 'Borrador',   colorClass: 'bg-gray-50 text-gray-500 dark:text-dark-tremor-content-subtle border-gray-200 dark:border-dark-tremor-border' },
};

function getStatus(status: string | undefined) {
  if (!status) return { label: 'Desconocido', colorClass: 'bg-gray-50 text-gray-500 dark:text-dark-tremor-content-subtle border-gray-200 dark:border-dark-tremor-border' };
  return STATUS_MAP[status.toUpperCase()] || { label: status, colorClass: 'bg-gray-50 text-gray-500 dark:text-dark-tremor-content-subtle border-gray-200 dark:border-dark-tremor-border' };
}
    ...
    return (
      <Card className="border border-gray-200 dark:border-dark-tremor-border shadow-sm rounded-xl">
        <div className="flex items-center gap-2 mb-4 pb-3 border-b border-gray-100 dark:border-dark-tremor-border">
          ...
          <h3 className="text-sm font-bold text-gray-900 dark:text-dark-tremor-content-strong">Historial de Órdenes</h3>
        </div>
        <div className="flex flex-col items-center justify-center p-12 bg-gradient-to-br from-white to-gray-50/50 dark:from-dark-tremor-background dark:to-dark-tremor-background-subtle rounded-lg border border-dashed border-gray-200 dark:border-dark-tremor-border">
          ...
          <p className="text-gray-500 dark:text-dark-tremor-content-subtle font-medium">No hay órdenes registradas</p>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50/80 dark:bg-dark-tremor-background-subtle border-b border-gray-100 dark:border-dark-tremor-border">
              ...
              <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 dark:text-dark-tremor-content-subtle uppercase tracking-wider">Entrega Est.</th>
              <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 dark:text-dark-tremor-content-subtle uppercase tracking-wider">Total</th>
            ...
            {paginatedOrders.map((order) => {
              ...
              return (
                <tr key={order.id} className="hover:bg-gray-50/50 dark:hover:bg-dark-tremor-background-subtle transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-600 dark:text-dark-tremor-content">{order.supplier?.name || '-'}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-gray-600 dark:text-dark-tremor-content">{formatDate(order.expectedDeliveryDate)}</td>
                  <td className="px-4 py-3 text-right">
                    <span className="text-gray-900 dark:text-dark-tremor-content-strong font-semibold">{formatCurrency(order.totalAmount)}</span>
                  </td>
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button
                ...
                className={`min-w-[2rem] h-8 px-2 rounded-lg text-xs font-semibold transition-colors ${
                  page === currentPage ? 'bg-indigo-600 text-white' : 'text-gray-600 dark:text-dark-tremor-content hover:bg-gray-200'
                }`}
              >
                {page}
              </button>
