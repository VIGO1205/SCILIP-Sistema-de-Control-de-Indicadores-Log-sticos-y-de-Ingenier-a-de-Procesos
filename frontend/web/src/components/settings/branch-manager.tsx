  ...
  return (
    <div ref={containerRef} className="relative">
      <div
        ...
        className="flex items-center justify-between w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white cursor-pointer hover:border-gray-300 transition-colors"
      >
          ...
          <div className="p-2 border-b border-gray-100">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Buscar..."
                className="w-full pl-8 pr-7 py-1.5 text-sm border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-green-400 focus:border-green-400"
              />
              {query && (
                <button
                  type="button"
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((opt) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-sm hover:bg-green-50 transition-colors ${opt === value ? 'bg-green-50 text-green-700 font-medium' : 'text-gray-700'}`}
                >
                  {opt}
                </button>
              ))
      ...
      {showForm && (
        <div className="border border-gray-200 rounded-xl p-4 bg-gray-50/50 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-gray-600 uppercase mb-1">Codigo *</label>
            ...
            <div>
              ...
              <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="block w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-green-100 focus:border-green-400" placeholder="Sede Bogota" />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-gray-600 uppercase mb-1">Pais</label>
              <Combobox
                value={form.country}
            ...
            <div>
              <label className="block text-[11px] font-semibold text-gray-600 uppercase mb-1">Telefono</label>
            ...
            <div>
              ...
              <input value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="block w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-green-100 focus:border-green-400" placeholder="sucursal@email.com" />
            </div>
            <div className="col-span-2">
              <label className="block text-[11px] font-semibold text-gray-600 uppercase mb-1">Direccion</label>
              <div className="flex">
                <input
                  value={form.address}
            ...
            <div key={branch.id} className="flex items-center justify-between p-3 rounded-xl border border-gray-100 hover:border-gray-200 transition-colors bg-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-500">{branch.code} {branch.city ? `\u00B7 ${branch.city}` : ''} {branch.country ? `\u00B7 ${branch.country}` : ''}</p>
                </div>
              </div>
